...
```
The estimated coefficient is `r round(DD$coefficients[[2]], 2)`, meaning that on average houses that are near the incinerator decrease in value by `r -1 * round(DD$coefficients[[2]], 2)`. 
...
nearinc <- DD_ci_95[2,]
low <- round(DD_ci_95[[1]], 2)
high <- round(DD_ci_95[[2]], 2)
```
