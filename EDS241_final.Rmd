...
mod_DD <- lm_robust(data = DD_data, formula = rprice ~ binary_time + nearinc +
                      dummy_var)
summary(mod_DD)
```
The estimated coefficient is `r round(mod_DD$coefficients[[4]], 2)`, meaning that on average houses that are near the incinerator decrease in value by `r -1 * round(mod_DD$coefficients[[4]], 2)`. 
...
Because our p-value is `r hypo$'Pr(>Chisq)'[2]`, we can reject the null hypothesis that all coefficients on housing and lot characteristics are jointly equal to 0. 
